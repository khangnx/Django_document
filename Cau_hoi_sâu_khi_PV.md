...
### 5. HTTP request status trong body JSON
- **Status code HTTP** (ví dụ: `200 OK`, `404 Not Found`, `500 Internal Server Error`) luôn nằm trong **header của response** và là chuẩn bắt buộc.
- **Body JSON** có thể chứa thêm trường `status`, `code`, `message` để mô tả chi tiết hơn:
  ```json
  {
    "status": 404,
